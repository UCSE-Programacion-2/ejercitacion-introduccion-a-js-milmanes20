...
const resultadoHoisting = 'number';
...
function valoresDelIndiceConLetEnBucle() {
  ...
  return funciones.map((func) => func());
}
...
function primerMultiploEnIntervaloDoWhile(inicio, divisor, limiteSuperior) {
  ...
  let multiplo = Math.ceil(inicio / divisor) * divisor; // Primer múltiplo >= inicio
  do {
    if (multiplo > limiteSuperior) {
      return null; // No hay múltiplo válido dentro del límite
    }
    if (multiplo % divisor === 0) {
      return multiplo; // Encontramos el múltiplo
    }
    multiplo += divisor; // Siguiente múltiplo
  } while (true);
}
